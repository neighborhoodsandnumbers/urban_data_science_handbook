...
| 3.5 (Flows on the Circle) | Trig identities, periodicity, $S^1$ as topological space, Weierstrass substitution / $\int d\theta/(a - b\sin\theta)$ integral, asymptotic analysis of integrals near a square-root singularity | 2.2 (Calculus), 2.3 (ODEs) |
...
- **Session 8**: Wrote section 3.5 (Flows on the Circle), closing Chapter 3. Covers the topological shift from $\mathbb{R}$ to $S^1$ and the consequence that oscillations become generic in one dimension on a circle (the "no-cycles" theorem of Section 3.1 was a topological artifact, not a physical law). Sets up the uniform oscillator $\dot\theta = \omega$ as the Platonic baseline, then introduces the non-uniform oscillator $\dot\theta = \omega - a\sin\theta$ as the canonical model — identified as, up to rescaling, the overdamped pendulum with constant torque, the Josephson junction in the resistive regime, and the phase equation for a weakly forced oscillator. Three regimes ($a<\omega$, $a=\omega$, $a>\omega$) treated geometrically with a unifying inline plot of $f(\theta)$ for all three. The bifurcation is named as the **SNIC** (Saddle-Node on Invariant Circle). Period integral $T = 2\pi/\sqrt{\omega^2 - a^2}$ derived and interpreted, with the $(\omega-a)^{-1/2}$ divergence tied to the saddle-node normal form from Section 3.3 (the universal exponent carries over because the local dynamics in the bottleneck *are* a saddle-node). "Ghost of the saddle-node" terminology introduced as the organizing physical metaphor, with an explicit forward reference to critical slowing down as an early-warning signal in Chapter 7. Fireflies and entrainment treated as the real-world payoff: Buck (1988) for the Pteroptyx malaccae phenomenon, Hanson (1978) mentioned for the entrainment experiment, and the reduction $\dot\psi = \Delta\omega - A\sin\psi$ derived so the whole non-uniform-oscillator analysis transfers verbatim to phase-locking. Arnold tongue named; Mirollo-Strogatz (1990) pulse-coupling theorem cited with forward ref to Kuramoto in Chapter 10. Closing paragraph summarizes Chapter 3 and bridges to Chapter 4 (phase plane, limit cycles, Hopf). Word count ~2400, matching baseline. Two code blocks: (1) $f(\theta)$ for three values of $a/\omega$ showing fixed points being born, (2) period $T$ vs $a/\omega$ with the $1/\sqrt{\omega-a}$ asymptote overlaid.
  - **Citations**: added `buck1988synchronous` and `mirollo1990synchronization` to references.bib (24 total entries). Strogatz (2015) not cited inline in this section — the Buck and Mirollo-Strogatz references are the load-bearing ones; Strogatz's book is the pedagogical source for the non-uniform oscillator treatment but is already densely cited throughout Chapter 3.
  - **Design choices**:
    - Chose *not* to include an Arnold-tongue diagram / two-parameter picture in this section — those belong to Chapter 10 (Dynamics on Networks §10.5 Synchronization) or Chapter 4 (forced oscillators in the phase plane). Section 3.5 names the Arnold tongue but does not construct it, keeping the scope one-dimensional.
    - Did not re-derive the $\int d\theta/(\omega - a\sin\theta)$ integral in full — quoted the result and flagged the Weierstrass substitution as the standard approach in the Ch 2 prerequisites tracker. A full derivation would balloon the section without teaching a complex-systems idea.
    - Kept the firefly vignette grounded in one specific species and one specific experiment rather than sprawling across "oscillators everywhere in biology" — the style guide's "synthetic then real" pattern, tightly scoped.
    - The "ghost of the saddle-node" language is worth its weight: it gives students a memorable handle on critical slowing down and tees up Chapter 7's early-warning-signal machinery without pre-empting it.
  - **Chapter 3 now complete** — 5 sections + intro + lab stub. Total ~11,500 words across sections 3.1–3.5. Lab (`3_6_lab_bifurcation_diagrams.ipynb`) still to be filled in. The chapter arc (geometry → stability → bifurcations → imperfection → topology) closes cleanly and hands off to Chapter 4 with the $\mathbb{R}^2$ phase plane as the natural next topological expansion.
