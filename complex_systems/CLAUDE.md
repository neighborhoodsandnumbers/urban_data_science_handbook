...
| 3.5 (Flows on the Circle) | Trig identities, periodicity, $S^1$ as topological space, Weierstrass half-angle substitution, improper integrals for oscillation period, square-root singularity at SNIC | 2.2 (Calculus), 2.3 (ODEs) |
...
- **Session 8**: Wrote section 3.5 (Flows on the Circle), closing Chapter 3 and bridging to Part II's two-dimensional material.
  - Opens with the topological move: bend the real line into $S^1$ and oscillation becomes possible. Eliot's "in my beginning is my end" (East Coker, Four Quartets) as the cultural reference — earned, because the line is literally about cyclic time. Periodicity condition $f(\theta+2\pi) = f(\theta)$ as the geometric constraint on admissible vector fields.
  - Uniform rotation $\dot{\theta} = \omega$ as the prototype phase oscillator with forward refs to Kuramoto (Ch 10) and biological clocks (Ch 16). Then the canonical non-uniform oscillator $\dot{\theta} = \omega - a\sin\theta$ with its three regimes: perpetual non-uniform rotation ($a<\omega$), phase-locking ($a>\omega$), and the borderline SNIC bifurcation ($a=\omega$). Emphasized that SNIC is a saddle-node on an invariant circle — a bifurcation that *uses* the closed topology essentially, distinguishing it from the saddle-node on the line in Section 3.3.
  - Period calculation via Weierstrass half-angle substitution, yielding $T = 2\pi/\sqrt{\omega^2 - a^2}$. Highlighted the $1/\sqrt{\omega-a}$ divergence as the same square-root scaling that appeared at the saddle-node (3.3), emerging here via the bottleneck effect — trajectories crawling past the ghost of a fixed point. Forward ref to Ch 7 early warning signals via critical slowing down.
  - Synchronization subsection: derived the Adler equation for two coupled oscillators and showed it is the non-uniform oscillator in disguise ($\Delta\omega$ replacing $\omega$, coupling $a$ preserved). Three applications: Pteroptyx fireflies in Thailand/Malaysia (Buck & Buck 1968, the Science paper on synchronous flashing), Josephson junctions (DC vs AC Josephson effect as locked vs rotating regimes), and phase-locked loops. Phrased as "the simplest mathematical theory of synchronization" with explicit forward ref to Kuramoto (Ch 10) as the $N$-oscillator generalization.
  - Closes with the ceiling of 1D: even on $S^1$, nested cycles are forbidden by Picard-Lindelöf — genuine limit cycles, Hopf bifurcations, and the spiraling approach to equilibria require the plane. Bridges to Chapter 4 via phase reduction: 2D limit cycles inherit a phase on $S^1$, so the techniques of this section survive as leading-order theory.
  - Two code blocks: (1) three regimes comparison showing rotation vs SNIC crawl vs locking, (2) period scaling table demonstrating the accelerating divergence as $a/\omega \to 1$.
  - Added Adler (1946) and Buck & Buck (1968) to references.bib (24 total entries). Citation density for the section: Strogatz (primary), Adler, Buck & Buck.
  - Word count: 2305. Matches baseline (2200–2500) set by sections 3.1–3.4.
  - **Josephson dimensional consistency check**: initial draft had $\dot\phi = (2eV/\hbar) - I_c\sin\phi$, which mixes dimensions (frequency on left, current on right). Corrected to the schematic $\dot\phi \propto I - I_c\sin\phi$ for the overdamped RCSJ model with the key point — it's an Adler equation with drive in the $\omega$ role and $I_c$ in the $a$ role — stated clearly. Caught and fixed before commit.
  - **Chapter 3 complete**: all 5 expository sections (3.1 phase portraits, 3.2 stability, 3.3 bifurcations, 3.4 imperfect bifurcations, 3.5 flows on the circle) + chapter intro written. Lab 3.6 (bifurcation diagrams) is the remaining file and is an `.ipynb`; labs are typically written after a chapter arc is stable. Total chapter word count: ~11,500 across 5 sections + intro. 10 code blocks, consistent voice and forward-referencing throughout.
