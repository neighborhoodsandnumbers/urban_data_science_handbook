...
- **Session 8**: Wrote section 3.5 (Flows on the Circle). Closes Chapter 3 by bending the real line into $S^1$ and showing that the no-oscillation theorem of Section 3.1 depends on topology, not dynamics — once recurrence is allowed, oscillations appear in one dimension. Opens with T. S. Eliot's "In my end is my beginning" (Four Quartets, East Coker) as the cultural reference — chosen because it literally describes the topology of the circle. Covers the uniform oscillator $\dot{\theta} = \omega$ (baseline with rotation symmetry), then the non-uniform oscillator $\dot{\theta} = \omega - a\sin\theta$ as the canonical example, with the three names explicitly listed (Adler's equation, overdamped Josephson junction, overdamped pendulum with torque) so students recognize them downstream. Two regimes analyzed: drift ($\omega > a$) and phase locking ($a > \omega$), with the critical case $a = \omega$ identified as a saddle-node on the invariant circle (SNIC). Period computed by the improper integral $T = \int_0^{2\pi} d\theta/(\omega - a\sin\theta) = 2\pi/\sqrt{\omega^2 - a^2}$ with the $1/\sqrt{\omega - a}$ critical slowing explicitly connected back to the square-root scaling in 3.3 — the *same* universality making a second appearance. Synchronization section covers Huygens's "odd kind of sympathy" (1665 pendulum clocks on a shared beam), *Pteroptyx malaccae* firefly synchrony in Southeast Asia (with a Kuramoto-style equation as explicit preview of Ch 10), and Josephson junction Shapiro steps as the metrological application. Forward references: Ch 4 (limit cycles as genuine 2D phenomenon), Ch 7 (critical slowing as early-warning signal), Ch 10 (Kuramoto model), Ch 13 (critical phenomena). Three inline code blocks: (1) vector field + trajectories for three values of $a$, showing drift/bifurcation/locking, (2) period vs $a$ with exact and asymptotic curves demonstrating the $1/\sqrt{}$ divergence. Closes with a careful distinction between oscillations "inherited from topology" (this chapter) and genuine limit cycles that require open state space (Ch 4) — motivating the chapter-level transition. Single citation (Strogatz 2015). Word count: ~2324.
  - **Design choices**:
    - Chose T. S. Eliot rather than Camus's Sisyphus or the Ouroboros — Eliot's "In my end is my beginning" is the tightest literary statement of circular topology, and the Four Quartets reference signals that this book is literate without being showy about it.
    - Listed all three names of the non-uniform oscillator (Adler, Josephson, pendulum) up front rather than scattering them through examples — students should recognize these are the same equation in different clothes. This is the normal-form / universality argument from 3.3 made concrete.
    - Kept the SNIC acronym brief — students will encounter it again in Ch 7 (tipping) and Ch 10 (Kuramoto) where the machinery matters more. Introduced here, developed later.
    - Kuramoto-style equation written explicitly in the synchronization passage so that Chapter 10 can reference "the non-uniform oscillator generalized to N coupled phases" without needing to re-derive the single-oscillator case.
    - Critical slowing connected explicitly to 3.3 (same square-root) and 13 (critical phenomena), 7 (tipping EWS). This reinforces the universality thread that 3.3 began.
  - **Chapter 3 complete** — all 5 expository sections written (3.1 phase portraits, 3.2 stability, 3.3 bifurcations, 3.4 imperfect bifurcations, 3.5 flows on the circle). Lab 3.6 (`3_6_lab_bifurcation_diagrams.ipynb`) remains to be written. No new bibliography entries this session — the pedagogical load is carried by Strogatz (2015) and internal cross-references.
  - **Ch 2 prerequisites** confirmed for 3.5: trig identities (Weierstrass substitution for integral computation), periodicity of $S^1$, improper integral for period calculation. All already logged in the tracker.
